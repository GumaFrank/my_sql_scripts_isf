...
D_PROPOSAL_SUBMIT,
TOTAL_PREMIUM,
JHL_GEN_PKG.GET_DISCOUNT_PREMIUM( V_POLICY_NO) DISCOUNT_PREMIUM,
NVL (N_RECEIPT_AMT, 0) N_RECEIPT_AMT,
TOTAL_ANNUAL_PREM,
N_SUM_COVERED SUM_ASURRED,
V_PYMT_DESC PYMT_FREQ,
V_PMT_METHOD_NAME,
V_PLAN_CODE,
V_PLAN_NAME,
N_TERM,   
...
NOTES,D_PROPOSAL_DATE
  FROM (SELECT A.D_PROPOSAL_SUBMIT,
            ...
            (SELECT   RTRIM (XMLAGG (XMLELEMENT (E, '  ' || NT1.V_LASTUPD_USER || ' : ' ||NT1.V_NOTE_MESSAGE)).EXTRACT ('//text()'), ',')  NOTES 
            FROM PSDT_POLICY_NOTEPAD NT1
            WHERE NT1.V_POLICY_NO =A.V_POLICY_NO
            GROUP BY NT1.V_POLICY_NO) NOTES,
            TRUNC(A.D_PROPOSAL_DATE)  D_PROPOSAL_DATE
...
--               AND NVL(A.D_PROPOSAL_DATE,TO_DATE('01/01/2000','DD/MM/YYYY')) BETWEEN TO_DATE(DECODE(ParamDates.ProFromDate, '', '01/01/1900', ParamDates.ProFromDate),'DD/MM/YYYY') AND TO_DATE(DECODE(ParamDates.ProToDate, '', '01/01/2050', ParamDates.ProToDate),'DD/MM/YYYY')
               --And A.D_Proposal_Date Between '01-JAN-2017' AND '06-DEC-2017'
              -- AND TRUNC(A.D_PROPOSAL_SUBMIT) BETWEEN :P_FM_DT  AND  :P_TO_DT
               AND A.V_CNTR_STAT_CODE = G.V_STATUS_CODE 
               AND C.N_AGENT_NO NOT IN (1218, 28020, 22778,28779)
               AND A.V_POLICY_NO NOT IN
...
--                                                        AND NVL(A.D_PROPOSAL_DATE,TO_DATE('01/01/2000','DD/MM/YYYY')) BETWEEN TO_DATE(DECODE(ParamDates.ProFromDate, '', '01/01/1900', ParamDates.ProFromDate),'DD/MM/YYYY') AND TO_DATE(DECODE(ParamDates.ProToDate, '', '01/01/2050', ParamDates.ProToDate),'DD/MM/YYYY')
                                                        ...
                                                        --AND TRUNC(A.D_PROPOSAL_DATE) BETWEEN :P_FM_DT  AND  :P_TO_DT
                                                        AND A.V_CNTR_STAT_CODE =G.V_STATUS_CODE 
                                                        AND C.N_AGENT_NO IN (65620,72770, 74753)
                                                        AND A.V_POLICY_NO NOT IN
                                                               (SELECT N.V_POLICY_NO
                                                                  FROM GN_CONTRACT_STATUS_LOG N
                                                                 WHERE N.V_POLICY_NO =A.V_POLICY_NO
                                               ...
                                               WHERE 
--                                               (NVL(TRIM(UPPER(SUBSTR(AGENT, INSTR(AGENT,'(')+1, INSTR(AGENT,')')-INSTR(AGENT,'(')+1-2))),'X') LIKE NVL(:P_AGENCY,'%') OR 
--                                                            :P_AGENCY IS NULL)
                                                           
                                                           SUBSTR (AGENT, 1, INSTR (AGENT, '-') - 1)  NOT  LIKE '50%'
