...
/* Formatted on 23/02/2022 11:37:12 (QP5 v5.256.13226.35538) */
SELECT A.V_Policy_No,
       ...
       (SELECT V_COMPANY_NAME
         ...
         WHERE     I.V_COMPANY_CODE = CO.V_COMPANY_CODE
               AND I.V_COMPANY_BRANCH = CO.V_COMPANY_BRANCH)
          BANK_NAME,
       V_CARD_BANK_CODE BANK_CODE,
       V_CARD_BANK_BRANCH BANK_BRANCH,
       TO_CHAR (V_EMP_ACCT_NO) ACCOUNT_NO
  FROM Gnmt_Policy A,
       ...
       Gnmm_Plan_Master H,
       Ammm_Agent_Master V,
       PPDT_POLICY_DEDUCTION_DETAILS I
 WHERE     A.V_Policy_No = B.V_Policy_No
       ...
       --AND I.V_POLICY_NO = 'IL201500564160'
       AND I.V_STATUS = 'A'
       AND C.N_Agent_No = V.N_Agent_No
       -- AND TRUNC (D_Policy_End_Date) BETWEEN :From_Date AND :TO_DATE
       --AND TRUNC (D_Policy_End_Date) BETWEEN :P_FROM_DATE AND :P_TO_DATE
       AND EXTRACT(YEAR FROM D_Policy_End_Date) >= 2023